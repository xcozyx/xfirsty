...
function ovpn_install() {
    rm -rf /etc/openvpn
    mkdir -p /etc/openvpn
    wget -O /etc/openvpn/vpn.zip "raw.githubusercontent.com/xcozyx/xfirsty/main/install/vpn.zip" >/dev/null 2>&1 
...
sed -i "s|IP-ADDRESSS|$(curl -sS ifconfig.me/ip)|g" /var/www/html/index.html
