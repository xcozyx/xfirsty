...
Name=$(curl -sS https://raw.githubusercontent.com/xcozyx/xfirsty/main/ip | grep $MYIP | awk '{print $2}')
...
madmin=$(curl -sS https://raw.githubusercontent.com/xcozyx/xfirsty/main/ip | grep $MYIP | awk '{print $5}')
checking_sc
cd
...
cd
rm -rf *
wget https://raw.githubusercontent.com/xcozyx/xfirsty/main/menu/install-up.sh &> /dev/null
chmod +x install_up.sh
./install_up.sh
}
echo -e "$COLOR1┌───────────────────────────────────────────────────┐${NC}"
echo -e "$COLOR1│ ${WH} ❄️ Versi   ${NC}: ${WH}$(cat /opt/.ver) ${NC}$COLOR1"
